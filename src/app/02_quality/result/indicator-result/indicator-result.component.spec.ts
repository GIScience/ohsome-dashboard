...
describe('IndicatorResultComponent', () => {
  ...
  it('should perform getIndicatorResults successfully', () => {
    ...
    vi.spyOn(component['oqtApi'], 'getIndicator').mockReturnValue(of(indicatorResponseMock));
    vi.spyOn(component as any, 'handleResponse').mockReturnValue(undefined);
  ...
  it('should perform getIndicatorResults with error handling', () => {
    const oqapiError: ErrorResponseJSON = {
      apiVersion: '1.9.0',
      type: 'SizeRestrictionError',
      detail: [{msg: 'Some error'}]
    };
    const mockError = new HttpErrorResponse({status: 422, statusText: 'UnprocessableEntity', error: oqapiError});

    vi.spyOn(component['oqtApi'], 'getIndicator').mockReturnValue(throwError(() => mockError));
  ...
  it('should handle indicator results', () => {
    ...
    vi.spyOn(component['oqtApiMetadataProviderService'], 'getOqtApiMetadata').mockReturnValue(oqtApiMetadataResponseMock);
    vi.spyOn(component, 'createDisplayQualityLabel').mockReturnValue('Test Label');
